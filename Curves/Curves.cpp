...
int main(int argc, const char* argv[]) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if ((ui_state.selected_item == 0 || ui_state.selected_item == keypoints.size() - 1) &&
            ...
            renderer.tangent = ui_state.selected_item;
        }
        else {
            renderer.tangent = -1;
        }
        ...
        if (ui_state.state == State::Nothing || ui_state.state == State::Highlighting) {
            ...
            if (ui_state.highlighting_item.first != -1) {
                renderer.highlight = ui_state.highlighting_item.first;
            ...
            for (int i = 0; i < renderer.tangent_lines.size(); i++) {
                float dist = glm::distance(glm::vec2(renderer.tangent_lines[i]), mouse);
            ...
            if (ui_state.tangent_highlighting_item.first != -1) {
                ui_state.state = State::TangentHighlighting;
                renderer.tangent_highlight = ui_state.tangent_highlighting_item.first;
            }
            else {
                renderer.tangent_highlight = -1;
            }
        }
        ...
        if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_1)) {
            if (ui_state.state == State::Nothing && !io.WantCaptureMouse) {
                ...
                renderer.highlight = ui_state.dragging_item;
            }
                ...
                ui_state.state = State::Tangent;
                ui_state.offset = glm::vec2(renderer.tangent_lines[ui_state.tangent_highlighting_item.first]) - mouse;
            }
                ...
                renderer.anchor = ui_state.selected_item;
                renderer.slope = pos;
                renderer.set_keypoints(keypoints);
            }
        }
